...
| **[Scriptlet][Scriptlet List]** | [Raw Link][Scriptlet Raw] <br><br> [Subscribe][Scriptlet Subscription] | **Description:** This filter list is for using [MrBukLau's Scriptlets][Scriptlet Link] in [uBlock Origin][uBlock Origin GitHub Link]. <br> **Disclaimer:** These filters were only tested on Google Chrome, which means that they may or may not work on other web browsers. To learn how to add these scriptlets when using uBlock Origin, click on this [link][uBlock Origin Scriptlet Guide]. <br> **Key Filters:** <br> - Change the image format and resolution for album artworks on Apple Music <br> - Redirect Reddit links to Old Reddit <br> - Translate everything to English on Apple Music
